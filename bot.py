...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Welcome! Use /books to see recommended JEE books.")
...
async def category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"📖 {category} Books (Page {page+1}):", reply_markup=InlineKeyboardMarkup(buttons))
...
if __name__ == "__main__":
    application.run_webhook(
        ...
        port=PORT,
        url_path=BOT_TOKEN,
        webhook_url=f"https://https://jee-bot-v1fa.onrender.com/{BOT_TOKEN}"
    )
